...
function DayPicker({
  ...
  function toggleWeek(weekDays: string[]) {
    ...
    const next = new Set(selected)
    if (allIn) weekDays.forEach(d => next.delete(d))
...
function ChildCard({
  ...
  const MY_ENTRIES_KEY = [{ _id: 'getApiV1VacationRegistrationByIdMyEntries', path: { id: window_.id } }] as const
  ...
  const isDirty =
    JSON.stringify(Array.from(selected).sort()) !== JSON.stringify([...(entry.selectedDates ?? [])].sort()) ||
    note !== (entry.note ?? '')
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        {saved && !isDirty && (
          <span className="text-xs text-green-700 bg-green-50 border border-green-200 px-2 py-0.5 rounded">Gemt</span>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {upsertMutation.isPending ? 'Gemmer…' : 'Gem indmelding'}
        </button>
      </form>
    </div>
  )
...
export default function ParentFerieindmeldingPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <h1 className="font-display text-2xl font-semibold text-gray-900 mb-6">Ferieindmelding</h1>

      {windows.length === 0 && (
        <p className="text-sm text-gray-500 py-8">Ingen åbne ferieindmeldinger.</p>
      )}
