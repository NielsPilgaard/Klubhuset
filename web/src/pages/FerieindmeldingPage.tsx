...
function GranularityBadge({ granularity }: { granularity: VacationRegistrationGranularity | undefined }) {
  return granularity === 'Days'
    ? <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800">Dage</span>
    : <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">Uger</span>
}

function OpenBadge({ isOpen }: { isOpen: boolean | undefined }) {
  return isOpen
    ? <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">Åben</span>
    : <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-600">Lukket</span>
}
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-2xl font-semibold text-gray-900">Ferieindmelding</h1>
        <button
      ...
      {windows.length === 0 && (
        <p className="text-sm text-gray-500 py-8">Ingen ferieindmeldinger oprettet endnu.</p>
      )}
          ...
          <div key={w.id} className="bg-white border border-gray-200 rounded-xl p-4">
            ...
            {deleteConfirm === w.id && w.id != null && (
              <div className="mt-3 pt-3 border-t border-gray-100 flex items-center gap-3">
                ...
                <button
                  onClick={() => { if (w.id == null) return; deleteMutation.mutate({ path: { id: w.id } }) }}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <h2 className="font-semibold text-gray-900 mb-4">Opret ferieindmelding</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    onChange={e => setForm(f => ({ ...f, careStartDate: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
                  />
                </div>
                <div className="flex-1">
                  ...
                  <input
                    ...
                    onChange={e => setForm(f => ({ ...f, careEndDate: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
                  />
                </div>
              </div>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="accent-brand-600"
                />
                <span className="text-sm text-gray-700">Åbn for indmeldinger med det samme</span>
              </label>
