...
export default function BeskederPage() {
	...
	const selectedInboxMsg =
		tab === "inbox" ? inbox.find((m) => m.id === selectedId) : null;
	const selectedSentMsg =
		tab === "sent" ? sent.find((m) => m.id === selectedId) : null;
									...
									<div className="flex items-start gap-3 mb-6 pb-4 border-b border-gray-200">
										<div className="flex items-center justify-center h-9 w-9 rounded-full bg-brand-100 text-brand-700 text-sm font-semibold shrink-0">
											{tab === "inbox"
												...
												: getInitials(
														(selectedMsg as SentMessageDto).recipientName,
													)}
										</div>
										<div className="min-w-0">
											{tab === "inbox" ? (
												<>
													...
													<p className="text-xs text-gray-500">
														{(selectedMsg as InboxMessageDto).senderType ===
														"Parent"
															? "Forælder"
															: "Medarbejder"}
														{" · "}
												...
												<>
													...
													<p className="text-xs text-gray-500">
														{(selectedMsg as SentMessageDto).recipientType ===
														"Parent"
															? "Forælder"
															: "Medarbejder"}
														{" · "}
							...
							{selectedRecipient ? (
								...
								<div className="relative">
									<input
										...
										onChange={(e) =>
											handleRecipientSearchChange(e.target.value)
										}
