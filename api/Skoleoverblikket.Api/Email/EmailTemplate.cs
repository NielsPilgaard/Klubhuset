...
internal static class EmailTemplate
{
	...
	/// <summary>
	/// Wraps pre-encoded HTML body content in the branded email shell.
	/// Callers are responsible for HTML-encoding all dynamic values before passing them in.
	/// </summary>
	internal static string Wrap(string title, string bodyHtml) => $$"""
		...
		<head>
		  ...
		  <title>{{title}} – Skoleoverblikket</title>
		  <style>
		    {{Styles}}
		  </style>
		</head>
