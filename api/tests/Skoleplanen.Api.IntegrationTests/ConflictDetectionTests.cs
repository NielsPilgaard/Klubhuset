...
public sealed class ConflictDetectionTests
{
    ...
    {
        var slot1 = await TestDataBuilder.CreateTimeSlotAsync(_factory.Services, _tenantId,
            new TimeOnly(8, 0), new TimeOnly(8, 45), sortOrder: 1);
        var slot2 = await TestDataBuilder.CreateTimeSlotAsync(_factory.Services, _tenantId,
        ...
        await UpsertSlotAsync(klass.Id, schema.Id, slot1.Id, weekday: 1, course.Id, teacher.Id);

        var response = await UpsertSlotAsync(klass.Id, schema.Id, slot2.Id, weekday: 1, course.Id, teacher.Id);
    ...
    public async Task TeacherDoubleBooked_WhenAssignedToTwoOverlappingSlots()
    {
        ...
        var slot2 = await TestDataBuilder.CreateTimeSlotAsync(_factory.Services, _tenantId,
            new TimeOnly(10, 0), new TimeOnly(10, 45), sortOrder: 4);
        ...
        await UpsertSlotAsync(klass.Id, schema.Id, slot1.Id, weekday: 2, course.Id, teacher.Id);

        var response = await UpsertSlotAsync(klass.Id, schema.Id, slot2.Id, weekday: 2, course.Id, teacher.Id);
        await Assert.That(response.StatusCode).IsEqualTo(HttpStatusCode.OK);
    ...
    {
        var slot1 = await TestDataBuilder.CreateTimeSlotAsync(_factory.Services, _tenantId,
            new TimeOnly(11, 0), new TimeOnly(11, 45), sortOrder: 5);
        var slot2 = await TestDataBuilder.CreateTimeSlotAsync(_factory.Services, _tenantId,
            new TimeOnly(11, 0), new TimeOnly(11, 45), sortOrder: 6);
        ...
        await UpsertSlotAsync(klass.Id, schema.Id, slot1.Id, weekday: 3, course.Id, teacher1.Id, roomId: room.Id);

        var response = await UpsertSlotAsync(klass.Id, schema.Id, slot2.Id, weekday: 3, course.Id, teacher2.Id, roomId: room.Id);
        var result = await response.Content.ReadFromJsonAsync<SlotsAndConflictsDto>();

        await Assert.That(result!.Conflicts.Any(c => c.Type == ConflictType.RoomDoubleBooked)).IsTrue();
        await Assert.That(result.Conflicts.First(c => c.Type == ConflictType.RoomDoubleBooked).ResourceId).IsEqualTo(room.Id);
    }
    ...
    {
        var slot1 = await TestDataBuilder.CreateTimeSlotAsync(_factory.Services, _tenantId,
            new TimeOnly(13, 0), new TimeOnly(13, 45), sortOrder: 7);
        var slot2 = await TestDataBuilder.CreateTimeSlotAsync(_factory.Services, _tenantId,
            new TimeOnly(13, 0), new TimeOnly(13, 45), sortOrder: 8);
        ...
        await UpsertSlotAsync(klass.Id, schema.Id, slot1.Id, weekday: 4, course.Id, teacher.Id);
        await UpsertSlotAsync(klass.Id, schema.Id, slot2.Id, weekday: 4, course.Id, teacher.Id);

        var response = await _client.PostAsync(
            $"/api/v1/classes/{klass.Id}/schemas/{schema.Id}/complete", null);
    ...
    {
        var slot1 = await TestDataBuilder.CreateTimeSlotAsync(_factory.Services, _tenantId,
            new TimeOnly(14, 0), new TimeOnly(14, 45), sortOrder: 9);
    ...
    private async Task<HttpResponseMessage> UpsertSlotAsync(
        Guid classId, Guid schemaId, Guid timeSlotId, int weekday,
    ...
    private record SlotDto(Guid Id, Guid TimeSlotId, int Weekday,
        Guid CourseId, string CourseName, Guid TeacherId, string TeacherName,
        Guid? RoomId, string? RoomName, Guid? AideId, string? AideName);

    private record ConflictInfoDto(
        ConflictType Type, Guid SlotAId, Guid SlotBId,
        Guid ResourceId, string ResourceName, int Weekday,
