...
- Extend `StaaMaalMedController`'s `GetCoverage` response to include this signal. Concretely: add an `UncoveredLessonCount` (int) to `SubjectCoverageDto`, matching the DTO's existing grouping exactly. `ClassCoverageDto.UnexpectedGradeCategories` (categories taught at a grade that UVM doesn't define for it, e.g. Tysk in 3. klasse) is a separate `List<string>` with no `SubjectCoverageDto` row at all — uncovered lessons in an unexpected category would otherwise be silently dropped from the response. Extend `UnexpectedGradeCategories` to a `List<UnexpectedCategoryDto>` (or equivalent) carrying `(Category, UncoveredLessonCount)` so those categories aren't excluded from the signal. `CoverageResponseDto` needs no structural change beyond what flows through from the extended `SubjectCoverageDto`/`UnexpectedGradeCategories`.
